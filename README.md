...
- показали, что архитектура (FastAPI + Context Stuffing + confidence-
  ...
  занял замену `knowledge.json`, категорий и системного промпта, без
  единой правки схемы БД или API;
...
- усилили бизнес-историю: B2B SaaS для крипто-бирж — понятный,
  объёмный рынок с наглядной unit-экономикой (цена за тикет), который
  проще питчить жюри, чем внутренний сервис одного вуза.

Это осознанный творческий ход, а не смена курса из-за проблем с
исходной идеей — техническое ядро осталось прежним и полностью
рабочим.
...
Этот README — сокращённая выжимка для быстрого старта; за деталями и
формулировками для защиты — в PRD.
...
│   └── .env                # VITE_API_BASE_URL, VITE_USE_MOCK_API
│
└── docs/
...
cp .env.example .env   # заполнить OPENAI_API_KEY и при необходимости DATABASE_URL
