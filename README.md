...
`.env`-файлы backend и frontend в репозиторий не коммитятся (в `.gitignore`) —
создаются локально каждым разработчиком, переменные см. ниже.
...
# создать .env в корне проекта со своими значениями — см. "Переменные окружения" ниже
...
cd help-me
npm install
# создать .env со своими значениями — см. "Переменные окружения" ниже
npm run dev
```
...
`.env`-файлы нигде не коммитятся — создайте их локально по образцу ниже.

**Backend — создать `.env` в корне проекта**
